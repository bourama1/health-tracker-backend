...
const fetchTaskRewards = (taskId) => {
  return new Promise((resolve, reject) => {
    db.all(
      `SELECT * FROM todo_task_rewards WHERE task_id = ?`,
      [taskId],
      (err, rows) => {
        if (err) reject(err);
        else resolve(rows || []);
...
const fetchAllTaskRewards = (userId) => {
  return new Promise((resolve, reject) => {
    db.all(
      `SELECT tr.* FROM todo_task_rewards tr JOIN todo_tasks t ON tr.task_id = t.id WHERE t.user_id = ?`,
      [userId],
      (err, rows) => {
        if (err) reject(err);
        else resolve(rows || []);
...
exports.createTask = (req, res) => {
  ...
  if (!rewards || !Array.isArray(rewards) || rewards.length === 0) {
    return res
      .status(400)
      .json({ error: 'At least one stat reward is required' });
  }
    ...
    (err, task) => {
      ...
      const updateRewards = (cb) => {
        if (rewards && Array.isArray(rewards)) {
          db.run(
            `DELETE FROM todo_task_rewards WHERE task_id = ?`,
            [id],
            (err) => {
