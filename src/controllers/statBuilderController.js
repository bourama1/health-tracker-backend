...
const getUserOrCreate = (req, res, callback) => {
  ...
  db.run(
    `INSERT OR IGNORE INTO stat_builder_profile (user_id) VALUES (?)`,
    [userId],
    (err) => {
      if (err) return res.status(400).json({ error: err.message });
      db.run(
        `INSERT OR IGNORE INTO stat_builder_stats (user_id, stat_name, value) VALUES (?, 'strength', 1), (?, 'dexterity', 1), (?, 'constitution', 1), (?, 'intelligence', 1), (?, 'wisdom', 1), (?, 'charisma', 1)`,
...
exports.getData = (req, res) => {
  getUserOrCreate(req, res, (userId) => {
    ...
    db.get(
      `SELECT * FROM stat_builder_profile WHERE user_id = ?`,
      [userId],
      (err, row) => {
        if (err) return res.status(400).json({ error: err.message });
        profile = row;
        ...
        db.all(
          `SELECT * FROM stat_builder_stats WHERE user_id = ?`,
          [userId],
          (err, rows) => {
            if (err) return res.status(400).json({ error: err.message });
            stats = rows;
            ...
            db.all(
              `SELECT * FROM stat_builder_skills WHERE user_id = ? AND active = 1 ORDER BY stat_name, order_index`,
              [userId],
              (err, rows) => {
                if (err) return res.status(400).json({ error: err.message });
                skills = rows;
                ...
                db.all(
                  `SELECT * FROM stat_builder_unlocks WHERE user_id = ? ORDER BY xp_threshold`,
                  [userId],
                  (err, rows) => {
                    if (err)
...
exports.updateStats = (req, res) => {
  ...
  db.serialize(() => {
    keys.forEach((statName) => {
      ...
      db.run(
        `INSERT INTO stat_builder_stats (user_id, stat_name, value) VALUES (?, ?, ?) ON CONFLICT(user_id, stat_name) DO UPDATE SET value = ?, updated_at = CURRENT_TIMESTAMP`,
        [req.session.user.id, statName, val, val],
        (err) => {
          if (!err) completed++;
        }
      );
...
exports.createSkill = (req, res) => {
  ...
  if (!stat_name || !name)
    return res.status(400).json({ error: 'stat_name and name required' });
  if (!STATS.includes(stat_name))
    return res.status(400).json({ error: `Invalid stat: ${stat_name}` });

  db.get(
    `SELECT COALESCE(MAX(order_index), -1) + 1 as next_order FROM stat_builder_skills WHERE user_id = ? AND stat_name = ?`,
    [req.session.user.id, stat_name],
    (err, row) => {
      if (err) return res.status(400).json({ error: err.message });
      const order = row.next_order;
      db.run(
        `INSERT INTO stat_builder_skills (user_id, stat_name, name, difficulty, order_index) VALUES (?, ?, ?, ?, ?)`,
        [req.session.user.id, stat_name, name, difficulty || 1, order],
        function (err) {
          if (err) return res.status(400).json({ error: err.message });
...
exports.updateSkill = (req, res) => {
  ...
  if (stat_name !== undefined) {
    fields.push('stat_name = ?');
    params.push(stat_name);
  }
  ...
  db.run(
    `UPDATE stat_builder_skills SET ${fields.join(', ')} WHERE id = ? AND user_id = ?`,
    params,
    function (err) {
      if (err) return res.status(400).json({ error: err.message });
...
exports.deleteSkill = (req, res) => {
  ...
  const { id } = req.params;
  db.run(
    `UPDATE stat_builder_skills SET active = 0, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?`,
      ...
      const applyXpAndRespond = (newCompleted, logId) => {
                ...
                (err, profile) => {
                  ...
                  if (newCompleted) {
                    ...
                    if (newLevel > profile.level) {
                      db.run(
                        `UPDATE stat_builder_profile SET level = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?`,
                        [newLevel, req.session.user.id]
                      );
                  ...
                  res.json({
                    ...
                    profile,
                    leveledUp,
                    message: newCompleted ? 'Logged' : 'Log toggled',
                  });
                }
              );
            }
        ...
        if (xpDelta !== 0) {
          db.run(
            ...
            [xpDelta, req.session.user.id, skill.stat_name],
            () => {
              db.run(
      ...
      db.get(
        ...
        (err, existing) => {
          ...
          if (existing) {
            ...
            db.run(
              `INSERT INTO stat_builder_logs (user_id, skill_id, date, completed) VALUES (?, ?, ?, 1)`,
...
exports.calculateWeek = (req, res) => {
  ...
  db.all(
    `SELECT sl.skill_id, ss.stat_name, ss.difficulty, COUNT(*) as completions FROM stat_builder_logs sl JOIN stat_builder_skills ss ON sl.skill_id = ss.id WHERE sl.user_id = ? AND sl.date >= ? AND sl.date <= ? AND sl.completed = 1 GROUP BY sl.skill_id`,
    [req.session.user.id, from, to],
    (err, rows) => {
      if (err) return res.status(400).json({ error: err.message });
...
exports.setUnlock = (req, res) => {
  ...
  if (!xp_threshold)
    return res.status(400).json({ error: 'xp_threshold required' });

  db.get(
    `SELECT id FROM stat_builder_unlocks WHERE user_id = ?`,
    [req.session.user.id],
    (err, row) => {
      if (err) return res.status(400).json({ error: err.message });
      if (row) {
        db.run(
          `UPDATE stat_builder_unlocks SET xp_threshold = ?, reward_text = ? WHERE user_id = ?`,
        ...
        db.run(
          `INSERT INTO stat_builder_unlocks (user_id, xp_threshold, reward_text) VALUES (?, ?, ?)`,
...
exports.resetWeek = (req, res) => {
  if (!req.session.user)
    return res.status(401).json({ error: 'Not authenticated' });
  db.run(
    `DELETE FROM stat_builder_logs WHERE user_id = ?`,
    [req.session.user.id],
    function (err) {
      if (err) return res.status(400).json({ error: err.message });
